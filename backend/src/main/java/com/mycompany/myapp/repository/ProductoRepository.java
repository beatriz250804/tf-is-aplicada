...
public interface ProductoRepository extends JpaRepository<Producto, Long> {
    ...
    @Query(
        value = "select producto from Producto producto left join fetch producto.cliente",
        countQuery = "select count(producto) from Producto producto"
    )
    Page<Producto> findAllWithToOneRelationships(Pageable pageable);

    @Query("select producto from Producto producto left join fetch producto.cliente")
    List<Producto> findAllWithToOneRelationships();

    @Query("select producto from Producto producto left join fetch producto.cliente where producto.id =:id")
